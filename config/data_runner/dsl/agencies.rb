# frozen_string_literal: true

[
  'Agencies',
  {
    steps: {
      enabled: true,
      manual_steps: Workflow::MANUAL_STEPS,
      scheduled: {
        frequency: :daily,
        steps: Workflow::SCHEDULED_STEPS
      }
    },
    group: {
      name: 'chart_of_accounts'
    },
    sop: {
      title: 'How to Download Chart of Accounts Agencies',
      source_system: 'ACO',
      reference_url: 'http://acweb/cutoff',
      reference_title: 'Downloaded Agencies File',
      reference_path: :downloaded_file,
      instructions: [
        'View the Chart of Accounts publication date from the Auditor-Controller Office.',
        'DataRunner orchestrates reloading the Chart of Accounts.',
        'Verify the downloaded agencies.xlsx file is current.'
      ]
    },
    source: {
      url: 'http://acweb/cutoff/WP Department on AUD Web Page.xlsx',
      location: 'agencies.xlsx',
      local: 'agencies.xlsx',
      format: :xlsx,
      strategy: :http
    },
    to_csv: {
      sheet: 0,
      header_row: 6,
      data_row: 7
    },
    use_dsl: {
      dedup: true
    },
    header: [
      ['fiscal_year',     nil, 'nvarchar(4)',   'NOT NULL', nil],
      ['department',      'agency_id',   'nvarchar(3)',   'NOT NULL', nil],
      ['department_name', 'name',        'nvarchar(100)', 'NOT NULL', nil],
      ['long_name',       'long_name',   'nvarchar(100)', 'NOT NULL', nil],
      ['short_name',      'short_name',  'nvarchar(100)', 'NOT NULL', nil],
      ['active',          'active',      'bit',           'NOT NULL', nil]
    ],

    database_connections: [
      {
        host: 'GSASQL16',
        database: 'GSABSS',
        schema: 'dbo',
        table: 'agencies',
        inject: {
          mode: :truncate_insert
        }
      }
    ]
  }
]
