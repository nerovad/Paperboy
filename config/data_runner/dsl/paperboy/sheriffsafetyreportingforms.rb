...
[
  'Sheriffsafetyreportingforms',
  {
    steps: {
      ...
      scheduled: {
        enabled: false, # paused by Duplicate: this DSL truncates the table it loads
        frequency: :daily,
    ...
    source: {
      location: File.join(ENV.fetch('DATARUNNER_INBOX'), 'sheriffsafetyreportingforms.csv'),
      local: 'sheriffsafetyreportingforms.csv',
      format: :csv,
      strategy: :copy
    },
    to_csv: {
    ...
    database_connections: [
      {
        ...
        schema: 'dbo',
        table: 'sheriff_safety_reporting_forms',
