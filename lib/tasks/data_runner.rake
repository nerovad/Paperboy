...
namespace :DataRunner do
  ...
  task :to_csv, [:name] do |_task, args|
    selector = DataRunnerTaskHelpers.task_arg(args, allow_all: true)
    DataRunnerTaskHelpers.run_stage_or_orchestration(selector, :to_csv, 'to_csv.rb')
  end
  ...
  task :to_sql, [:name] do |_task, args|
    selector = DataRunnerTaskHelpers.task_arg(args, allow_all: true)
    DataRunnerTaskHelpers.run_stage_or_orchestration(selector, :to_sql, 'to_sql.rb')
  end
  ...
  task :table_drop, [:name] do |_task, args|
    selector = DataRunnerTaskHelpers.task_arg(args, allow_all: true)
    DataRunnerTaskHelpers.run_stage_or_orchestration(selector, :drop_table, 'drop_tables.rb')
  end

  desc 'Create configured SQL Server tables'
  task :table_create, [:name] do |_task, args|
    selector = DataRunnerTaskHelpers.task_arg(args, allow_all: true)
    DataRunnerTaskHelpers.run_stage_or_orchestration(selector, :create_table, 'create_tables.rb')
  end

  desc 'Apply locked DSL mappings to normalized CSV files'
  task :use_dsl, [:name] do |_task, args|
    selector = DataRunnerTaskHelpers.task_arg(args, allow_all: true)
    DataRunnerTaskHelpers.run_stage_or_orchestration(selector, :use_dsl, 'use_dsl.rb')
  end
  ...
  task :refresh, [:name] do |_task, args|
    ...
    if DataRunnerTaskHelpers.orchestrated?(selector)
      DataRunnerTaskHelpers.run_orchestrated_refresh(selector)
    else
