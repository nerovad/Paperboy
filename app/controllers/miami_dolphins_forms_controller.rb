class MiamiDolphinsFormsController < ApplicationController
  # Minimal controller for the two-page template (Employee Info + Agency Info)

  def new
    @miami_dolphins_form = MiamiDolphinsForm.new
  ...
  def create
    ...
    @miami_dolphins_form = MiamiDolphinsForm.new(miami_dolphins_form_params)
    @miami_dolphins_form.employee_id = employee_id if @miami_dolphins_form.respond_to?(:employee_id=)

    if @miami_dolphins_form.save
  ...
  def miami_dolphins_form_params
    # Only the baseline fields you asked for
    params.require(:miami_dolphins_form).permit(
