class PolarExpressFormsController < ApplicationController
  # Minimal controller for the two-page template (Employee Info + Agency Info)

  def new
    @polar_express_form = PolarExpressForm.new
  ...
  def create
    ...
    @polar_express_form = PolarExpressForm.new(polar_express_form_params)
    @polar_express_form.employee_id = employee_id if @polar_express_form.respond_to?(:employee_id=)

    if @polar_express_form.save
  ...
  def polar_express_form_params
    # Only the baseline fields you asked for
    params.require(:polar_express_form).permit(
