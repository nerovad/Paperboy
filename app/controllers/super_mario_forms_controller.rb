class SuperMarioFormsController < ApplicationController
  # Minimal controller for the two-page template (Employee Info + Agency Info)

  def new
    @super_mario_form = SuperMarioForm.new
  ...
  def create
    ...
    @super_mario_form = SuperMarioForm.new(super_mario_form_params)
    @super_mario_form.employee_id = employee_id if @super_mario_form.respond_to?(:employee_id=)

    if @super_mario_form.save
  ...
  def super_mario_form_params
    # Only the baseline fields you asked for
    params.require(:super_mario_form).permit(
