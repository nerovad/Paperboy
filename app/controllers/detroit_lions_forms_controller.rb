class DetroitLionsFormsController < ApplicationController
  # Minimal controller for the two-page template (Employee Info + Agency Info)

  def new
    @detroit_lions_form = DetroitLionsForm.new
  ...
  def create
    ...
    @detroit_lions_form = DetroitLionsForm.new(detroit_lions_form_params)
    @detroit_lions_form.employee_id = employee_id if @detroit_lions_form.respond_to?(:employee_id=)

    if @detroit_lions_form.save
  ...
  def detroit_lions_form_params
    # Only the baseline fields you asked for
    params.require(:detroit_lions_form).permit(
