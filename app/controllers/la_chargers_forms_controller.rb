class LaChargersFormsController < ApplicationController
  # Minimal controller for the two-page template (Employee Info + Agency Info)

  def new
    @la_chargers_form = LaChargersForm.new
  ...
  def create
    ...
    @la_chargers_form = LaChargersForm.new(la_chargers_form_params)
    @la_chargers_form.employee_id = employee_id if @la_chargers_form.respond_to?(:employee_id=)

    if @la_chargers_form.save
  ...
  def la_chargers_form_params
    # Only the baseline fields you asked for
    params.require(:la_chargers_form).permit(
