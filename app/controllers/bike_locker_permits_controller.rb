class BikeLockerPermitsController < ApplicationController
  # Minimal controller for the two-page template (Employee Info + Agency Info)

  def new
    @bike_locker_permit = BikeLockerPermit.new
  ...
  def create
    ...
    @bike_locker_permit = BikeLockerPermit.new(bike_locker_permit_params)
    @bike_locker_permit.employee_id = employee_id if @bike_locker_permit.respond_to?(:employee_id=)

    if @bike_locker_permit.save
  ...
  def bike_locker_permit_params
    # Only the baseline fields you asked for
    params.require(:bike_locker_permit).permit(
