class BuffaloBillsFormsController < ApplicationController
  # Minimal controller for the two-page template (Employee Info + Agency Info)

  def new
    @buffalo_bills_form = BuffaloBillsForm.new
  ...
  def create
    ...
    @buffalo_bills_form = BuffaloBillsForm.new(buffalo_bills_form_params)
    @buffalo_bills_form.employee_id = employee_id if @buffalo_bills_form.respond_to?(:employee_id=)

    if @buffalo_bills_form.save
  ...
  def buffalo_bills_form_params
    # Only the baseline fields you asked for
    params.require(:buffalo_bills_form).permit(
