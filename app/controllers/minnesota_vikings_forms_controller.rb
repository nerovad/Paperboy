class MinnesotaVikingsFormsController < ApplicationController
  # Minimal controller for the two-page template (Employee Info + Agency Info)

  def new
    @minnesota_vikings_form = MinnesotaVikingsForm.new
  ...
  def create
    ...
    @minnesota_vikings_form = MinnesotaVikingsForm.new(minnesota_vikings_form_params)
    @minnesota_vikings_form.employee_id = employee_id if @minnesota_vikings_form.respond_to?(:employee_id=)

    if @minnesota_vikings_form.save
  ...
  def minnesota_vikings_form_params
    # Only the baseline fields you asked for
    params.require(:minnesota_vikings_form).permit(
