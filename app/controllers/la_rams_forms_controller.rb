class LaRamsFormsController < ApplicationController
  # Minimal controller for the two-page template (Employee Info + Agency Info)

  def new
    @la_rams_form = LaRamsForm.new
  ...
  def create
    ...
    @la_rams_form = LaRamsForm.new(la_rams_form_params)
    @la_rams_form.employee_id = employee_id if @la_rams_form.respond_to?(:employee_id=)

    if @la_rams_form.save
  ...
  def la_rams_form_params
    # Only the baseline fields you asked for
    params.require(:la_rams_form).permit(
