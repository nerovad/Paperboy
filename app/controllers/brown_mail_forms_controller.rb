class BrownMailFormsController < ApplicationController
  # Minimal controller for the two-page template (Employee Info + Agency Info)

  def new
    @brown_mail_form = BrownMailForm.new
  ...
  def create
    ...
    @brown_mail_form = BrownMailForm.new(brown_mail_form_params)
    @brown_mail_form.employee_id = employee_id if @brown_mail_form.respond_to?(:employee_id=)

    if @brown_mail_form.save
  ...
  def brown_mail_form_params
    # Only the baseline fields you asked for
    params.require(:brown_mail_form).permit(
