class NewYorkGiantsFormsController < ApplicationController
  # Minimal controller for the two-page template (Employee Info + Agency Info)

  def new
    @new_york_giants_form = NewYorkGiantsForm.new
  ...
  def create
    ...
    @new_york_giants_form = NewYorkGiantsForm.new(new_york_giants_form_params)
    @new_york_giants_form.employee_id = employee_id if @new_york_giants_form.respond_to?(:employee_id=)

    if @new_york_giants_form.save
      # Keep success behavior simple for the template; you can extend per form.
      # Route to supervisor for approval
@new_york_giants_form.update(status: :pending)
...
redirect_to form_success_path, notice: 'Form submitted and routed to your supervisor for approval.', allow_other_host: false, status: :see_other
  ...
  def new_york_giants_form_params
    # Only the baseline fields you asked for
    params.require(:new_york_giants_form).permit(
