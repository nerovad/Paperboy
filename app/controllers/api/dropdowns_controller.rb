# frozen_string_literal: true

module Api
  class DropdownsController < ApplicationController
    def divisions
      divisions = Coa::Division.where(Agency: params[:agency])
                               .select(:Division, :LongName)
                               .distinct
                               .map { |d| { value: d.Division, label: d.LongName.to_s } }

      render json: divisions
    end

    def departments
      departments = Coa::Department.where(Agency: params[:agency], Division: params[:division])
                                   .select(:Department, :LongName)
                                   .distinct
                                   .map { |d| { value: d.Department, label: d.LongName.to_s } }

      render json: departments
    end

    def units
      units = Coa::Unit.where(Agency: params[:agency], Division: params[:division], Department: params[:department])
                       .select(:Unit, :LongName)
                       .distinct
                       .map { |u| { value: u.Unit, label: "#{u.Unit} #{u.LongName}" } }

      render json: units
    end
  end
end
