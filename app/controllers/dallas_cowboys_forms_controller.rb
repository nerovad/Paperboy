class DallasCowboysFormsController < ApplicationController
  # Minimal controller for the two-page template (Employee Info + Agency Info)

  def new
    @dallas_cowboys_form = DallasCowboysForm.new
  ...
  def create
    ...
    @dallas_cowboys_form = DallasCowboysForm.new(dallas_cowboys_form_params)
    @dallas_cowboys_form.employee_id = employee_id if @dallas_cowboys_form.respond_to?(:employee_id=)

    if @dallas_cowboys_form.save
  ...
  def dallas_cowboys_form_params
    # Only the baseline fields you asked for
    params.require(:dallas_cowboys_form).permit(
