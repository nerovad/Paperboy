class PrincessBrideFormsController < ApplicationController
  # Minimal controller for the two-page template (Employee Info + Agency Info)

  def new
    @princess_bride_form = PrincessBrideForm.new
  ...
  def create
    ...
    @princess_bride_form = PrincessBrideForm.new(princess_bride_form_params)
    @princess_bride_form.employee_id = employee_id if @princess_bride_form.respond_to?(:employee_id=)

    if @princess_bride_form.save
  ...
  def princess_bride_form_params
    # Only the baseline fields you asked for
    params.require(:princess_bride_form).permit(
