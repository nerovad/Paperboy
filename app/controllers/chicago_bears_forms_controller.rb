class ChicagoBearsFormsController < ApplicationController
  # Minimal controller for the two-page template (Employee Info + Agency Info)

  def new
    @chicago_bears_form = ChicagoBearsForm.new
  ...
  def create
    ...
    @chicago_bears_form = ChicagoBearsForm.new(chicago_bears_form_params)
    @chicago_bears_form.employee_id = employee_id if @chicago_bears_form.respond_to?(:employee_id=)

    if @chicago_bears_form.save
  ...
  def chicago_bears_form_params
    # Only the baseline fields you asked for
    params.require(:chicago_bears_form).permit(
