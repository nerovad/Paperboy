class SeattleSeahawksFormsController < ApplicationController
  # Minimal controller for the two-page template (Employee Info + Agency Info)

  def new
    @seattle_seahawks_form = SeattleSeahawksForm.new
  ...
  def create
    ...
    @seattle_seahawks_form = SeattleSeahawksForm.new(seattle_seahawks_form_params)
    @seattle_seahawks_form.employee_id = employee_id if @seattle_seahawks_form.respond_to?(:employee_id=)

    if @seattle_seahawks_form.save
  ...
  def seattle_seahawks_form_params
    # Only the baseline fields you asked for
    params.require(:seattle_seahawks_form).permit(
