...
#
# These strings are stored verbatim in critical_information_reportings.location
# and in critical_information_authorizations.location, so they are data, not
# labels: fixing a typo here (CAMRILLO, MOORPARK AVEE, PONDEROSA DR .) orphans
# every report and authorization already recorded against it. Add new sites at
# the end of the city's run; retire one only by also migrating the rows that
# point at it.
class CriticalInformationLocation
  ALL = [
  ...
  def self.options
    ALL.map { |location| [location, location] }
  end

  def self.include?(location)
    ALL.include?(location.to_s)
  end
end
