...
module Forms
  class Template < ApplicationRecord
    ...
    def self.model_name = ActiveModel::Name.new(self, nil, 'FormTemplate')
    ...
    has_many :form_fields, class_name: 'Forms::Field', foreign_key: :form_template_id, dependent: :destroy
    has_many :routing_steps, -> { order(:step_number) }, class_name: 'Forms::TemplateRoutingStep',
                                                         foreign_key: :form_template_id, dependent: :destroy
    has_many :statuses, -> { order(:position) }, class_name: 'Forms::TemplateStatus',
                                                 foreign_key: :form_template_id, dependent: :destroy
    has_many :copy_recipients, -> { order(:position, :id) }, class_name: 'Forms::TemplateCopyRecipient',
                                                             foreign_key: :form_template_id, dependent: :destroy
    has_many :email_steps, -> { order(:position, :id) }, class_name: 'Forms::TemplateEmailStep',
                                                         foreign_key: :form_template_id, dependent: :destroy
    ...
    def assign_reference_prefix
      ...
      candidate = Reference::PREFIX_SEEDS[class_name] || Reference.derive_prefix(class_name)
