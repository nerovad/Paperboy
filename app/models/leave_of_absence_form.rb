class LeaveOfAbsenceForm < ApplicationRecord
  ...
  def acceptable_doctors_note_attachment_files
    ...
    errors.add(:doctors_note_attachment, 'can have a maximum of 10 files') if doctors_note_attachment.count > 10

    doctors_note_attachment.each do |file|
      errors.add(:doctors_note_attachment, 'must be a JPEG, PNG, GIF, or PDF') unless file.content_type.in?(%w[image/jpeg image/png image/gif application/pdf])

      errors.add(:doctors_note_attachment, 'file size must be less than 10MB') if file.byte_size > 10.megabytes
    end
  end
end
