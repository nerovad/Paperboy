...
ActiveRecord::Schema[8.0].define(version: 20_260_827_000_001) do
  create_table 'Employee_Groups', force: :cascade do |t|
    t.integer 'EmployeeID', null: false
    t.bigint 'GroupID', null: false
    t.datetime 'Assigned_At', precision: nil, default: -> { 'getdate()' }
    t.integer 'Assigned_By'
    t.index %w[EmployeeID GroupID], name: 'idx_employee_groups_on_employee_group', unique: true
  end

  create_table 'Group_Permissions', primary_key: %w[GroupID Permission_Type Permission_Key], force: :cascade do |t|
  ...
  create_table 'bike_lockers', force: :cascade do |t|
    ...
    t.datetime 'created_at', null: false
    t.datetime 'updated_at', null: false
    t.index %w[lot_id locker_number], name: 'index_bike_lockers_on_lot_id_and_locker_number', unique: true
    t.index %w[lot_id status], name: 'index_bike_lockers_on_lot_id_and_status'
  ...
  create_table 'data_runner_group_run_items', force: :cascade do |t|
    ...
    t.index %w[group_run_id position], name: 'idx_group_run_items_position', unique: true
  ...
  create_table 'data_runner_group_runs', force: :cascade do |t|
    ...
    t.datetime 'created_at', null: false
    t.datetime 'updated_at', null: false
    t.index %w[group_name status], name: 'index_data_runner_group_runs_on_group_name_and_status'
  ...
  create_table 'form_submission_copies', force: :cascade do |t|
    ...
    t.index %w[submission_type submission_id recipient_employee_id], name: 'index_form_submission_copies_unique_per_recipient', unique: true
    t.index %w[submission_type submission_id], name: 'index_form_submission_copies_on_submission'
  end
  ...
  create_table 'form_visibility_grants', force: :cascade do |t|
    ...
    t.index %w[grantee_type group_id], name: 'index_form_visibility_grants_on_grantee_type_and_group_id'
  end
  ...
  create_table 'osha_300a_entries', force: :cascade do |t|
    ...
    t.index %w[osha_establishment_id year], name: 'index_osha_300a_entries_on_osha_establishment_id_and_year', unique: true
  ...
  create_table 'tasks', primary_key: %w[agency_id task_id], force: :cascade do |t|
