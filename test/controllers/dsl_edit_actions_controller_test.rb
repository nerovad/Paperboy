...
class DslEditActionsControllerTest < ActionController::TestCase
  ...
  test 'edit action bar places save between edit and delete' do
    ...
    assert_select '.hot-menu input.btn.approve[value=?][form=?]', 'Save', 'dsl-source-form'
    assert_select '.hot-menu a.btn[href=?]', data_runner_dsl_path('employees'), text: 'Cancel'
    assert_select '.hot-menu form[action=?] .btn.deny', data_runner_dsl_path('employees'), false
    assert_select '.hot-menu .btn', text: 'Outputs', count: 0
    assert_select '.hot-menu summary.btn.approve', text: 'Run Task', count: 0
